...
class UseToolViewModel : ViewModel() {
    ...
    private val _distributors = MutableStateFlow<List<Locker>>(emptyList())
    val distributors: StateFlow<List<Locker>> = _distributors
    ...
    fun findDistributorById(id: String): Locker? = _distributors.value.find { it.id == id }
}
